...
#if !defined(RTOS)
static void test2() {
  FILE* out_file_ = fopen("d:\\rcrtc\\media\\suzie_qcif.h264", "wb");
  if (out_file_ == NULL) {
    return;
  }
  FILE* yuv_file_ = fopen("d:\\rcrtc\\media\\suzie_qcif.yuv", "rb");
  if (yuv_file_ == NULL) {
    fclose(out_file_);
    return;
  }
  ...
  pic.pData[2] = pic.pData[1] + (width * height >> 2);
  for (int num = 0; num < total_num; num++) {
    size_t readed = fread(buf, 1, width * height * 3 / 2, yuv_file_);
    if (readed != width * height * 3 / 2) {
      break;
    }
    //prepare input data
    rv = (*encoder_)->EncodeFrame(encoder_, &pic, &info);
